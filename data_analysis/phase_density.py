...
for t in range(0, 4001, 500):
    ...
    h_b = plt.hist2d(v_b_para, v_b_per, bins=200, range=[[-12, 12], [0, 15]])
    ...
    col = ax.pcolormesh(X_b, Y_b, np.log10((value_b / (index * yedges_b[1:]))).T, shading='gouraud', cmap=palette, vmax=0)
    ...
    # ax.set_xlim(-12, 12)
    ax.axis('scaled')
    ax.set_title(r'$t\Omega_i=$' + str(t * 0.01))
    ax.set_xlabel('$v_{||}/v_A$')
